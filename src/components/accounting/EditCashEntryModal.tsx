...
import { updateCashBookEntryAction, getAccountingLookups } from "@/modules/accounting/actions";
...
interface EditCashEntryModalProps {
  ...
  entry: any; // CashEntryRow object
  onSuccess?: () => void;
}

export function EditCashEntryModal({ open, onOpenChange, entry, onSuccess }: EditCashEntryModalProps) {
  ...
  // Set values when entry is provided or changed
  useEffect(() => {
    if (open && entry) {
      setEntryDate(entry.entryDate ? new Date(entry.entryDate).toISOString().slice(0, 10) : "");
      setType(entry.type);
      setAmount(entry.amount ? String(parseFloat(entry.amount)) : "");
  ...
  const handleSubmit = () => {
    ...
    if (!entry?.id) {
      setError("Invalid transaction selected for edit.");
      return;
    }
    ...
    startTransition(async () => {
      const res = await updateCashBookEntryAction(entry.id, payload);
      if (res.success) {
        ...
        setError(res.error || "Failed to update cash transaction.");
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Edit Cash Book Entry</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4 pt-2">
          ...
          <div>
            ...
            <Input
              placeholder="e.g. Cleared monthly internet, paint retail billing..."
              value={description}
              onChange={(e) => setDescription(e.target.value)}
            />
          </div>

          {error && <p className="text-xs font-bold text-red-650 mt-2">{error}</p>}
        </div>
