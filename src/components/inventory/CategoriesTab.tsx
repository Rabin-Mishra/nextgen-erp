...
export function CategoriesTab({ initialCategories, pagination, searchQuery }: CategoriesTabProps) {
  ...
  const handleToggleActive = async (category: CategoryData) => {
    const originalState = category.isActive;
    ...
    try {
      await updateCategory(category.id, { isActive: nextState });
      toast.success(
      ...
      setCategories((prev) =>
        prev.map((c) => (c.id === category.id ? { ...c, isActive: originalState } : c))
      );
  ...
  const handleDeleteClick = (category: CategoryData) => {
    ...
    if (productsCount > 0) {
      toast.error(`Cannot delete. ${productsCount} products are assigned to this category.`);
      return;
    }
          ...
          <TableBody>
            {filteredCategories.length === 0 ? (
              ...
              filteredCategories.map((c) => (
                <TableRow key={c.id} className="hover:bg-zinc-50/50 dark:hover:bg-zinc-900/30">
                  {/* Category Name */}
                  ...
                  <TableCell className="text-zinc-500 dark:text-zinc-400 max-w-[320px] truncate">
                    {c.description || <span className="italic text-zinc-300">No description provided</span>}
                  </TableCell>
      ...
      <Dialog open={isDeleteOpen} onOpenChange={(val) => !val && setIsDeleteOpen(false)}>
        <DialogContent className="max-w-md rounded-2xl p-6">
          ...
          <div className="py-4 text-sm text-zinc-600 dark:text-zinc-400">
            Are you sure you want to delete category <strong className="text-zinc-950 dark:text-zinc-50">"{categoryToDelete?.name}"</strong>? This will permanently erase the category from the database.
          </div>
