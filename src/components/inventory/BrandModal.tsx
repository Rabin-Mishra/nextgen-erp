...
import { createBrand, updateBrand } from "@/modules/inventory/actions";
import { toast } from "sonner";
import { Award, Loader2, Edit3 } from "lucide-react";
...
export function BrandModal({ isOpen, onClose, onSuccess, brand }: BrandModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.message || `Failed to ${isEdit ? "update" : "create"} brand.`);
    } finally {
      setLoading(false);
    }
  };
      ...
      <DialogContent className="max-w-md rounded-2xl p-6">
        <DialogHeader>
          ...
          <p className="text-xs text-zinc-400 mt-1">
            {isEdit
              ? "Update brand details and descriptions."
              : "Define a new product manufacturer brand."}
          </p>
        </DialogHeader>
